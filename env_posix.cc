...
public:
	...
	virtual Status Read(size_t n, Slice* result, char* scratch)
	{
		Status s;
		//文件读取
		size_t r = fread_ulocked(scratch, 1, n, file_);
		*result = Slice(scratch, r);
		if(r < n){
			if(!feof(file_)){ //未到末尾且数据长度没有达到要读的长度,这是个IO读错误
...
private:
	std::string filename_; //用于产生错误提示信息
	FILE* file_;
};
...
public:
	...
	virtual Status Read(uint64_t offset, size_t n, Slice* result, char* scratch) const 
	{
		...
		if(offset + n > length_){
			...
			*result = Slice(reinterpret_cast<char*>(mmapped_region_) + offset, n); //直接内存隐射读取
		}
...
{
public:
	PosixWriteableFile(const std::string& fname, FILE* f) : filename_(fname), file_(f){};
	virtual ~PosixWriteableFile()
	{
		fclose(file_);
	};
	...
	virtual Status Sync()
	{
		Status s = SyncDirIfManifest(); //对文件进行SYNC检查
		if (!s.ok()) {
			return s;
		}
...
public:
	PosixEnv();
	virtual ~PosixEnv(){
		fprintf(stderr, "Destroy Env::Default()\n");
		abort();
	};
	//顺序读取
	...
	virtual Status NewRandomAccessFile(const std::string& fname, RandomAccessFile** result)
	{
		*result = NULL;
		...
		else if(mmap_limit_.Acquire()){ //尝试mmap读模式
			uint64_t size;
	...
	//写
	virtual Status NewWriteableFile(const std::string& fname, WritableFile** result)
	{
		Status s;
		...
		if(f == NULL){
			...
			*result = new PosixWriteableFile(fname, f);
		}
	}
	...
	virtual Status CreateDir(const std::string& fname)
	{
		Status result;
		if(unlink(fname.c_str()) != 0){
			result = IOError(fname, errno);
