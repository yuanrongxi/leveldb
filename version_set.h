#ifndef __LEVEL_DB_VERSION_SET_H_
#define __LEVEL_DB_VERSION_SET_H_
...
public:
	void AddIterators(const ReadOptions& opt, std::vector<Iterator*>* iters);
	Status Get(const ReadOptions& opt, const LookupKey& key, std::string* val, GetStats* stats);
...
private:
	Iterator* NewConcatenatingIterator(const ReadOptions& opt, int level) const;
...
{
public:
	VersionSet(const std::string& dbname, const Options* opt, TableCache* table_cache, const InternalKeyComparator* icmp);
	~VersionSet();
