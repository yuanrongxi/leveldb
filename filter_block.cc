...
void FilterBlockBuilder::GenerateFilter()
{
	...
	for(size_t i = 0; i < num_keys; i ++){
		...
		size_t length = start_[i + 1] - start[i];
...
bool FilterBlockReader::KeyMayMatch(uint64_t block_offset, const Slice& key)
{
	...
	if(index < num_){
		uint32_t start = DecodeFixed32(offset_ + index * 4);     //entry的开始位置
		uint32_t limit = DecodeFixed32(offset_ + index * 4 + 4); //entry的结束位置(其实就是下一个entry的开始位置)
